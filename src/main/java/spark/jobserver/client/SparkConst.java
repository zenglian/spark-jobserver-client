...
/**
 * Spark configurations. Generated from https://spark.apache.org/docs/latest/configuration.html
 */
@Data
...
public class SparkConst implements Mapping {
  ...
  // collect). Should be at
  // least 1M, or 0 for unlimited. Jobs will be aborted if the total size is above this limit.
  // Having a high limit
  // may cause out-of-memory errors in driver (depends on spark.driver.memory and memory overhead
  // of objects in
  ...
  // SparkContext, instances of
  // these classes will be created and registered with Spark's listener bus. If a class has a
  // single-argument
  // constructor that accepts a SparkConf, that constructor will be called; otherwise, a
  // zero-argument constructor
  // will be called. If no valid constructor can be found, the SparkContext creation will fail
  // with an exception.
  ...
  // stored on disk.
  // This should be on a fast, local disk in your system. It can also be a comma-separated list
  // of multiple
  // directories on different disks. NOTE: In Spark 1.0 and later this will be overridden by
  // SPARK_LOCAL_DIRS
  ...
  //The deploy mode of Spark driver program, either "client" or "cluster", Which means to launch
  // driver program
  ...
  //Application information that will be written into Yarn RM log/HDFS audit log when
  // stillRunning on Yarn/HDFS.
  // Its length depends on the Hadoop configuration hadoop.caller.context.max.size. It should be
  // concise, and
  // typically can have up to 50 characters.
  @SerializedName("spark.log.callerContext")
  private String logCallerContext;

  //If true, restarts the driver automatically if it fails with a non-zero exit status. Only has
  // effect in Spark
  ...
  // logging. Note that it
  // is illegal to set maximum heap size (-Xmx) settings with this option. Maximum heap size
  // settings can be set
  // with spark.driver.memory in the cluster mode and through the --driver-memory command line
  ...
  // classes in the
  // driver. This feature can be used to mitigate conflicts between Spark's dependencies and user
  // dependencies. It
  ...
  // logging. Note that it is
  // illegal to set Spark properties or maximum heap size (-Xmx) settings with this option. Spark
  // properties should
  // be set using a SparkConf object or the spark-defaults.conf file used with the spark-submit
  // script. Maximum
  ...
  //Enable executor log compression. If it is enabled, the rolled executor logs will be
  // compressed. Disabled by
  // default.
  @SerializedName("spark.executor.logs.rolling.enableCompression")
  private boolean executorLogsRollingEnableCompression = false;

  //Set the max size of the file in bytes by which the executor logs will be rolled over. Rolling
  // is disabled by
  ...
  //Set the strategy of rolling of executor logs. By default it is disabled. It can be set to
  // "time" (time-based
  // rolling) or "size" (size-based rolling). For "time", use spark.executor.logs.rolling.time
  // .interval to set the
  // rolling interval. For "size", use spark.executor.logs.rolling.maxSize to set the maximum
  // file size for rolling.
  @SerializedName("spark.executor.logs.rolling.strategy")
  private String executorLogsRollingStrategy;

  //Set the time interval by which the executor logs will be rolled over. Rolling is disabled by
  // default. Valid
  ...
  //(Experimental) Same functionality as spark.driver.userClassPathFirst, but applied to executor
  // instances.
  ...
  // executor
  // environments contain sensitive information. When this regex matches a property key or value,
  // the value is
  ...
  //Enable profiling in Python worker, the profile result will show up by sc.show_profiles(), or
  // it will be
  // displayed before the driver exiting. It also can be dumped into disk by sc.dump_profiles
  // (path). If some of the
  // profile results had been displayed manually, they will not be displayed automatically before
  // driver exiting.
  ...
  //The directory which is used to dump the profile result before driver exiting. The results
  // will be dumped as
  // separated file for each RDD. They can be loaded by ptats.Stats(). If this is specified, the
  // profile result
  // will not be displayed automatically.
  @SerializedName("spark.python.profile.dump")
  private String pythonProfileDump;

  //Amount of memory to use per python worker process during aggregation, in the same format as
  // JVM memory strings
  // (e.g. 512m, 2g). If the memory used during aggregation goes above this amount, it will spill
  // the data into disks.
  ...
  // need to fork() a
  // Python process for every tasks. It will be very useful if there is large broadcast, then the
  // broadcast will
  ...
  // classpaths. The
  // coordinates should be groupId:artifactId:version. If spark.jars.ivySettings is given
  // artifacts will be
  // resolved according to the configuration in the file, otherwise artifacts will be searched
  // for in the local
  // maven repo, then maven central and finally any additional remote repositories given by the
  // command-line option
  ...
  //Path to an Ivy settings file to customize resolution of jars specified using spark.jars
  // .packages instead of the
  // built-in defaults, such as maven central. Additional repositories given by the command-line
  // option
  // --repositories will also be included. Useful for allowing Spark to resolve artifacts from
  // behind a firewall e
  // .g. via an in-house artifact server like Artifactory. Details on the settings file format
  // can be found at
  ...
  //Maximum size of map outputs to fetch simultaneously from each reduce task. Since each output
  // requires us to
  // create a buffer to receive it, this represents a fixed memory overhead per reduce task, so
  // keep it small
  ...
  // When the number of
  // hosts in the cluster increase, it might lead to very large number of in-bound connections to
  // one or more
  // nodes, causing the workers to fail under load. By allowing it to limit the number of fetch
  // requests, this
  // scenario can be mitigated.
  @SerializedName("spark.reducer.maxReqsInFlight")
  private String reducerMaxReqsInFlight = "Int.MaxValue";

  //Whether to compress map output files. Generally a good idea. Compression will use spark.io
  // .compression.codec.
  @SerializedName("spark.shuffle.compress")
  private boolean shuffleCompress = true;

  //Size of the in-memory buffer for each shuffle file output stream. These buffers reduce the
  ...
  //(Netty only) Fetches that fail due to IO-related exceptions are automatically retried if this
  // is set to a
  // non-zero value. This retry logic helps stabilize large shuffles in the face of long GC
  // pauses or transient
  ...
  // large clusters. For
  // clusters with many hard disks and few hosts, this may result in insufficient concurrency to
  // saturate all
  ...
  // block transfer.
  // For environments where off-heap memory is tightly limited, users may wish to turn this off
  // to force all
  ...
  //(Netty only) How long to wait between retries of fetches. The maximum delay caused by
  // retrying is 15 seconds by
  ...
  // executors so the
  // executors can be safely removed. This must be enabled if spark.dynamicAllocation.enabled is
  // "true". The
  // external shuffle service must be set up in order to enable it. See dynamic allocation
  ...
  //Whether to compress data spilled during shuffles. Compression will use spark.io.compression
  // .codec.
  ...
  // size accurately if
  // it's above this config. This helps to prevent OOM by avoiding underestimating shuffle block
  // size when fetch
  // shuffle blocks.
  @SerializedName("spark.shuffle.accurateBlockThreshold")
  private String shuffleAccurateBlockThreshold = "100 * 1024 * 1024";

  //Enable IO encryption. Currently supported by all modes except Mesos. It's recommended that
  // RPC encryption be
  ...
  // described in the
  // KeyGenerator section of the Java Cryptography Architecture Standard Algorithm Name
  // Documentation.
  @SerializedName("spark.io.encryption.keygen.algorithm")
  private String ioEncryptionKeygenAlgorithm = "HmacSHA1";

  //Whether to compress logged events, if spark.eventLog.enabled is true. Compression will use
  // spark.io.compression
  // .codec.
  @SerializedName("spark.eventLog.compress")
  private boolean eventLogCompress = false;

  //Base directory in which Spark events are logged, if spark.eventLog.enabled is true. Within
  // this base directory,
  // Spark creates a sub-directory for each application, and logs the events specific to the
  // application in this
  // directory. Users may want to set this to a unified location like an HDFS directory so
  // history files can be
  ...
  //How many jobs the Spark UI and status APIs remember before garbage collecting. This is a
  // target maximum, and
  // fewer elements may be retained in some circumstances.
  @SerializedName("spark.ui.retainedJobs")
  private long uiRetainedJobs = 1000;

  //How many stages the Spark UI and status APIs remember before garbage collecting. This is a
  // target maximum, and
  // fewer elements may be retained in some circumstances.
  @SerializedName("spark.ui.retainedStages")
  private long uiRetainedStages = 1000;

  //How many tasks the Spark UI and status APIs remember before garbage collecting. This is a
  // target maximum, and
  // fewer elements may be retained in some circumstances.
  ...
  // Spark master will
  // reverse proxy the worker and application UIs to enable access without requiring direct
  // access to their hosts.
  // Use it with caution, as worker and application UI will not be accessible directly, you will
  // only be able to
  // access them through spark master/proxy public URL. This setting affects all the workers and
  // application UIs
  ...
  //This is the URL where your proxy is running. This URL is for proxy which is running in front
  // of Spark Master.
  // This is useful when running proxy for authentication e.g. OAuth proxy. Make sure this is a
  // complete URL
  ...
  //Show the progress bar in the console. The progress bar shows the progress of stages that run
  // for longer than
  // 500ms. If multiple stages run at the same time, multiple progress bars will be displayed on
  // the same line.
  ...
  // Compression will use spark
  // .io.compression.codec.
  @SerializedName("spark.broadcast.compress")
  private boolean broadcastCompress = true;

  //The codec used to compress internal data such as RDD partitions, event log, broadcast
  // variables and shuffle
  // outputs. By default, Spark provides three codecs: lz4, lzf, and snappy. You can also use
  ...
  //Block size used in LZ4 compression, in the case when LZ4 compression codec is used. Lowering
  // this block size
  ...
  //Block size used in Snappy compression, in the case when Snappy compression codec is used.
  // Lowering this block
  ...
  // necessary if your
  // object graphs have loops and useful for efficiency if they contain multiple copies of the
  // same object. Can be
  ...
  //Whether to require registration with Kryo. If set to 'true', Kryo will throw an exception if
  // an unregistered
  // class is serialized. If set to false (the default), Kryo will write unregistered class names
  // along with each
  // object. Writing class names can cause significant performance overhead, so enabling this
  // option can enforce
  ...
  //If you use Kryo serialization, give a comma-separated list of classes that register your
  // custom classes with
  // Kryo. This property is useful if you need to register your classes in a custom way, e.g. to
  // specify a custom
  // field serializer. Otherwise spark.kryo.classesToRegister is simpler. It should be set to
  // classes that extend
  // KryoRegistrator. See the tuning guide for more details.
  @SerializedName("spark.kryo.registrator")
  private String kryoRegistrator;

  //Whether to use unsafe based Kryo serializer. Can be substantially faster by using Unsafe
  // Based IO.
  @SerializedName("spark.kryo.unsafe")
  private boolean kryoUnsafe = false;

  //Maximum allowable size of Kryo serialization buffer. This must be larger than any object you
  // attempt to
  // serialize and must be less than 2048m. Increase this if you get a "buffer limit exceeded"
  // exception inside Kryo.
  @SerializedName("spark.kryoserializer.buffer.max")
  private String kryoserializerBufferMax = "64m";

  //Initial size of Kryo's serialization buffer. Note that there will be one buffer per core on
  // each worker. This
  ...
  //Whether to compress serialized RDD partitions (e.g. for StorageLevel.MEMORY_ONLY_SER in Java
  // and Scala or
  // StorageLevel.MEMORY_ONLY in Python). Can save substantial space at the cost of some extra
  // CPU time.
  ...
  // objects to prevent
  // writing redundant data, however that stops garbage collection of those objects. By calling
  // 'reset' you flush
  // that info from the serializer, and allow old objects to be collected. To turn off this
  // periodic reset set it
  ...
  // frequently spills
  // and cached data eviction occur. The purpose of this config is to set aside memory for
  // internal metadata, user
  // data structures, and imprecise size estimation in the case of sparse, unusually large
  // records. Leaving this at
  // the default value is recommended. For more detail, including important information about
  // correctly tuning JVM
  ...
  // region set aside by
  // s​park.memory.fraction. The higher this is, the less working memory may be available to
  // execution and tasks
  // may spill to disk more often. Leaving this at the default value is recommended. For more
  // detail, see this
  // description.
  ...
  //The absolute amount of memory in bytes which can be used for off-heap allocation. This
  // setting has no impact on
  // heap memory usage, so if your executors' total memory consumption must fit within some hard
  // limit then be sure
  // to shrink your JVM heap size accordingly. This must be set to a positive value when spark
  // .memory.offHeap
  // .enabled=true.
  @SerializedName("spark.memory.offHeap.size")
  private long memoryOffHeapSize = 0;

  //​Whether to enable the legacy memory management mode used in Spark 1.5 and before. The legacy
  // mode rigidly
  // partitions the heap space into fixed-size regions, potentially leading to excessive spilling
  // if the
  // application was not tuned. The following deprecated memory fraction configurations are not
  // read unless this is
  // enabled: spark.shuffle.memoryFraction
  @SerializedName("spark.memory.useLegacyMode")
  private boolean memoryUseLegacyMode = false;

  //(deprecated) This is read only if spark.memory.useLegacyMode is enabled. Fraction of Java
  // heap to use for
  // aggregation and cogroups during shuffles. At any given time, the collective size of all
  // in-memory maps used
  // for shuffles is bounded by this limit, beyond which the contents will begin to spill to disk
  // . If spills are
  ...
  //(deprecated) This is read only if spark.memory.useLegacyMode is enabled. Fraction of Java
  // heap to use for
  // Spark's memory cache. This should not be larger than the "old" generation of objects in the
  // JVM, which by
  // default is given 0.6 of the heap, but you can increase it if you configure your own old
  // generation size.
  @SerializedName("spark.storage.memoryFraction")
  private double storageMemoryFraction = 0.6;

  //(deprecated) This is read only if spark.memory.useLegacyMode is enabled. Fraction of spark
  // .storage
  // .memoryFraction to use for unrolling blocks in memory. This is dynamically allocated by
  // dropping existing
  ...
  // executor failures are
  // replenished if there are any existing available replicas. This tries to get the replication
  // level of the block
  // to the initial number.
  @SerializedName("spark.storage.replication.proactive")
  private boolean storageReplicationProactive = false;

  //Size of each piece of a block for TorrentBroadcastFactory. Too large a value decreases
  // parallelism during
  // broadcast (makes it slower); however, if it is too small, BlockManager might take a
  // performance hit.
  @SerializedName("spark.broadcast.blockSize")
  private String broadcastBlockSize = "4m";

  //The number of cores to use on each executor. In standalone and Mesos coarse-grained modes,
  // setting this
  // parameter allows an application to run multiple executors on the same worker, provided that
  // there are enough
  ...
  private int executorCores = 1; //"1 in YARN mode, all the available cores on the worker in
  ...
  // that the executor is
  // still alive and update it with metrics for in-progress tasks. spark.executor
  // .heartbeatInterval should be
  ...
  // that belong to the
  // same application, which can improve task launching performance when stillRunning many
  // executors on the same
  // host. If set to false, these caching optimizations will be disabled and all executors will
  // fetch their own
  // copies of files. This optimization may be disabled in order to use Spark local directories
  // that reside on NFS
  ...
  //Whether to overwrite files added through SparkContext.addFile() when the target file exists
  // and its contents do
  ...
  //The estimated cost to open a file, measured by the number of bytes could be scanned in the
  // same time. This is
  // used when putting multiple files into a partition. It is better to over estimate, then the
  // partitions with
  ...
  //If set to true, clones a new Hadoop Configuration object for each task. This option should be
  // enabled to work
  // around Configuration thread-safety issues (see SPARK-2546 for more details). This is
  // disabled by default in
  // order to avoid unexpected performance regressions for jobs that are not affected by these
  // issues.
  @SerializedName("spark.hadoop.cloneConf")
  private boolean hadoopCloneConf = false;

  //If set to true, validates the output specification (e.g. checking if the output directory
  // already exists) used
  // in saveAsHadoopFile and other variants. This can be disabled to silence exceptions due to
  // pre-existing output
  // directories. We recommend that users do not disable this except if trying to achieve
  // compatibility with
  // previous versions of Spark. Simply use Hadoop's FileSystem API to delete output directories
  // by hand. This
  // setting is ignored for jobs generated through Spark Streaming's StreamingContext, since data
  // may need to be
  ...
  //Size of a block above which Spark memory maps when reading a block from disk. This prevents
  // Spark from memory
  // mapping very small blocks. In general, memory mapping has high overhead for blocks close to
  // or below the page
  // size of the operating system.
  @SerializedName("spark.storage.memoryMapThreshold")
  private String storageMemoryMapThreshold = "2m";

  //The file output committer algorithm version, valid algorithm version number: 1 or 2. Version
  // 2 may have better
  // performance, but version 1 may handle failures better in certain situations, as per
  // MAPREDUCE-4815.
  ...
  //Maximum message size (in MB) to allow in "control plane" communication; generally only
  // applies to map output
  // size information sent between executors and the driver. Increase this if you are
  // stillRunning jobs with many
  ...
  //Driver-specific port for the block manager to listen on, for cases where it cannot use the
  // same configuration
  // as executors.
  @SerializedName("spark.driver.blockManager.port")
  private String driverBlockManagerPort = "(value of spark.blockManager.port)";

  //Hostname or IP address where to bind listening sockets. This config overrides the
  // SPARK_LOCAL_IP environment
  // variable (see below).
  @SerializedName("spark.driver.bindAddress")
  private String driverBindAddress = "(value of spark.driver.host)";

  //Hostname or IP address for the driver. This is used for communicating with the executors and
  // the standalone
  // Master.
  @SerializedName("spark.driver.host")
  private String driverHost = "(local hostname)";

  //Port for the driver to listen on. This is used for communicating with the executors and the
  // standalone Master.
  @SerializedName("spark.driver.port")
  private String driverPort = "(random)";

  //Default timeout for all network interactions. This config will be used in place of spark.core
  // .connection.ack
  // .wait.timeout, spark.storage.blockManagerSlaveTimeoutMs, spark.shuffle.io.connectionTimeout,
  // spark.rpc
  ...
  //Maximum number of retries when binding to a port before giving up. When a port is given a
  // specific value (non
  // 0), each subsequent retry will increment the port used in the previous attempt by 1 before
  // retrying. This
  // essentially allows it to try a range of ports from the start port specified to port +
  // maxRetries.
  @SerializedName("spark.port.maxRetries")
  private long portMaxRetries = 16;

  //Number of times to retry before an RPC task gives up. An RPC task will run at most times of
  // this number.
  ...
  //When stillRunning on a standalone deploy cluster or a Mesos cluster in "coarse-grained"
  // sharing mode, the
  // maximum amount of CPU cores to request for the application from across the cluster (not from
  // each machine). If
  // not set, the default will be spark.deploy.defaultCores on Spark's standalone cluster
  // manager, or infinite (all
  // available cores) on Mesos.
  @SerializedName("spark.cores.max")
  private String coresMax = "(not set)";

  //How long to wait to launch a data-local task before giving up and launching it on a
  // less-local node. The same
  // wait will be used to step through multiple locality levels (process-local, node-local,
  // rack-local and then
  // any). It is also possible to customize the waiting time for each level by setting spark
  // .locality.wait.node,
  // etc. You should increase this setting if your tasks are long and see poor locality, but the
  // default usually
  // works well.
  ...
  // (resources are
  // executors in yarn mode, CPU cores in standalone mode and Mesos coarsed-grained mode ['spark
  // .cores.max' value
  // is total expected resources for Mesos coarse-grained mode] ) to wait for before scheduling
  // begins. Specified
  // as a double between 0.0 and 1.0. Regardless of whether the minimum ratio of resources has
  // been reached, the
  // maximum amount of time it will wait before scheduling begins is controlled by config spark
  // .scheduler
  // .maxRegisteredResourcesWaitingTime.
  @SerializedName("spark.scheduler.minRegisteredResourcesRatio")
  private String schedulerMinRegisteredResourcesRatio = "0.8 for YARN mode; 0.0 for standalone "
      + "mode and Mesos "
      + "coarse-grained mode";

  //The scheduling mode between jobs submitted to the same SparkContext. Can be set to FAIR to
  // use fair sharing
  ...
  // due to too many
  // task failures. The blacklisting algorithm can be further controlled by the other "spark
  // .blacklist"
  // configuration options.
  @SerializedName("spark.blacklist.enabled")
  private boolean blacklistEnabled = false;

  //(Experimental) How long a node or executor is blacklisted for the entire application, before
  // it is
  ...
  //(Experimental) For a given task, how many times it can be retried on one node, before the
  // entire node is
  // blacklisted for that task.
  @SerializedName("spark.blacklist.task.maxTaskAttemptsPerNode")
  private long blacklistTaskMaxTaskAttemptsPerNode = 2;

  //(Experimental) How many different tasks must fail on one executor, within one stage, before
  // the executor is
  ...
  //(Experimental) How many different executors are marked as blacklisted for a given stage,
  // before the entire node
  ...
  // before the executor
  // is blacklisted for the entire application. Blacklisted executors will be automatically added
  // back to the pool
  // of available resources after the timeout specified by spark.blacklist.timeout. Note that
  // with dynamic
  // allocation, though, the executors may get marked as idle and be reclaimed by the cluster
  // manager.
  ...
  // before the node is
  // blacklisted for the entire application. Blacklisted nodes will be automatically added back
  // to the pool of
  // available resources after the timeout specified by spark.blacklist.timeout. Note that with
  // dynamic allocation,
  // though, the executors on the node may get marked as idle and be reclaimed by the cluster
  // manager.
  ...
  // executors when
  // they are blacklisted. Note that, when an entire node is added to the blacklist, all of the
  // executors on that
  // node will be killed.
  @SerializedName("spark.blacklist.killBlacklistedExecutors")
  private boolean blacklistKillBlacklistedExecutors = false;

  //If set to "true", performs speculative execution of tasks. This means if one or more tasks
  // are stillRunning
  ...
  // failures spread
  // across different tasks will not cause the job to fail; a particular task has to fail this
  // number of attempts.
  ...
  // will be monitored
  // by the executor until that task actually finishes executing. See the other spark.task.reaper
  // .* configurations
  // for details on how to control the exact behavior of this monitoring. When set to false (the
  // default), task
  ...
  // will poll the
  // status of killed tasks. If a killed task is still stillRunning when polled then a warning
  // will be logged and,
  // by default, a thread-dump of the task will be logged (this thread dump can be disabled via
  // the spark.task
  ...
  // executor JVM will kill
  // itself if a killed task has not stopped stillRunning. The default value, -1, disables this
  // mechanism and
  // prevents the executor from self-destructing. The purpose of this setting is to act as a
  ...
  //If dynamic allocation is enabled and an executor which has cached data blocks has been idle
  // for more than this
  ...
  //Whether Spark acls should be enabled. If enabled, this checks to see if the user has access
  // permissions to view
  // or modify the job. Note this requires the user to be known, so if the user comes across as
  // null no checks are
  ...
  //Comma separated list of users/administrators that have view and modify access to all Spark
  // jobs. This can be
  // used if you run on a shared cluster and have a set of administrators or devs who help debug
  // when things do not
  ...
  //Comma separated list of groups that have view and modify access to all Spark jobs. This can
  // be used if you have
  // a set of administrators or developers who help maintain and debug the underlying
  // infrastructure. Putting a "*"
  // in the list means any user in any group can have the privilege of admin. The user groups are
  // obtained from the
  // instance of the groups mapping provider specified by spark.user.groups.mapping. Check the
  // entry spark.user
  ...
  // org.apache.spark
  // .security.GroupMappingServiceProvider which can configured by this property. A default unix
  // shell based
  // implementation is provided org.apache.spark.security.ShellBasedGroupsMappingProvider which
  // can be specified to
  // resolve a list of groups for a user. Note: This implementation supports only a Unix/Linux
  // based environment.
  // Windows environment is currently not supported. However, a new platform/protocol can be
  // supported by
  ...
  // mechanism. This is
  // useful when the application is connecting to old shuffle services that do not support the
  // internal Spark
  // authentication protocol. On the server side, this can be used to block older clients from
  // authenticating
  ...
  //Enable encrypted communication when authentication is enabled. This is supported by the block
  // transfer service
  ...
  //How long for the connection to wait for ack to occur before timing out and giving up. To
  // avoid unwilling
  ...
  // user that started
  // the Spark job has access to modify it (kill it for example). Putting a "*" in the list means
  // any user can have
  // access to modify it.
  @SerializedName("spark.modify.acls")
  private String modifyAcls;

  //Comma separated list of groups that have modify access to the Spark job. This can be used if
  // you have a set of
  // administrators or developers from the same team to have access to control the job. Putting a
  // "*" in the list
  // means any user in any group has the access to modify the Spark job. The user groups are
  // obtained from the
  // instance of the groups mapping provider specified by spark.user.groups.mapping. Check the
  // entry spark.user
  // .groups.mapping for more details.
  ...
  //Comma separated list of filter class names to apply to the Spark web UI. The filter should be
  // a standard javax
  // servlet Filter. Parameters to each filter can also be specified by setting a java system
  // property of:
  ...
  // user that started
  // the Spark job has view access. Putting a "*" in the list means any user can have view access
  // to this Spark job.
  @SerializedName("spark.ui.view.acls")
  private String uiViewAcls;

  //Comma separated list of groups that have view access to the Spark web ui to view the Spark
  // Job details. This
  // can be used if you have a set of administrators or developers or users who can monitor the
  // Spark job submitted
  // . Putting a "*" in the list means any user in any group can view the Spark job details on
  // the Spark web ui.
  ...
  //A comma separated list of ciphers. The specified ciphers must be supported by JVM. The
  // reference list of
  // protocols one can find on this page. Note: If not set, it will use the default cipher suites
  // of JVM.
  ...
  //A path to a key-store file. The path can be absolute or relative to the directory where the
  // component is
  // started in.
  ...
  //A protocol name. The protocol must be supported by JVM. The reference list of protocols one
  ...
  //A path to a trust-store file. The path can be absolute or relative to the directory where the
  // component is
  // started in.
  ...
  //Enables or disables Spark Streaming's internal backpressure mechanism (since 1.5). This
  // enables the Spark
  // Streaming to control the receiving rate based on the current batch scheduling delays and
  // processing times so
  // that the system receives only as fast as the system can process. Internally, this
  // dynamically sets the maximum
  // receiving rate of receivers. This rate is upper bounded by the values spark.streaming
  // .receiver.maxRate and
  ...
  //Interval at which data received by Spark Streaming receivers is chunked into blocks of data
  // before storing them
  // in Spark. Minimum recommended - 50 ms. See the performance tuning section in the Spark
  ...
  // Effectively, each stream
  // will consume at most this number of records per second. Setting this configuration to 0 or a
  // negative number
  // will put no limit on the rate. See the deployment guide in the Spark Streaming programing
  // guide for mode details.
  @SerializedName("spark.streaming.receiver.maxRate")
  private String streamingReceiverMaxRate = "not set";

  //Enable write ahead logs for receivers. All the input data received through receivers will be
  // saved to write
  // ahead logs that will allow it to be recovered after driver failures. See the deployment
  // guide in the Spark
  ...
  // Spark's memory. The
  // raw input data received by Spark Streaming is also automatically cleared. Setting this to
  // false will allow the
  // raw data and persisted RDDs to be accessible outside the streaming application as they will
  // not be cleared
  ...
  //If true, Spark shuts down the StreamingContext gracefully on JVM shutdown rather than
  // immediately.
  ...
  //Maximum number of consecutive retries the driver will make in order to find the latest
  // offsets on the leader of
  // each partition (a default value of 1 means that the driver will make a maximum of 2
  // attempts). Only applies to
  ...
  //Whether to close the file after writing a write ahead log record on the driver. Set this to
  // 'true' when you
  // want to use S3 (or any file system that does not support flushing) for the metadata WAL on
  // the driver.
  ...
  //Whether to close the file after writing a write ahead log record on the receivers. Set this
  // to 'true' when you
  // want to use S3 (or any file system that does not support flushing) for the data WAL on the
  // receivers.
  ...
  //Executable for executing sparkR shell in client modes for driver. Ignored in cluster modes.
  // It is the same as
  // environment variable SPARKR_DRIVER_R, but take precedence over it. spark.r.shell.command is
  // used for sparkR
  ...
  //Checkpoint interval for graph and message in Pregel. It used to avoid stackOverflowError due
  // to long lineage
  ...
  //The recovery mode setting to recover submitted Spark jobs with cluster mode when it failed
  // and relaunches. This
  ...
  //When `spark.deploy.recoveryMode` is set to ZOOKEEPER, this configuration is used to set the
  // zookeeper URL to
  // connect to.
